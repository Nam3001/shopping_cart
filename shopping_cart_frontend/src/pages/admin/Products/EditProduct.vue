  ...
  <AdminLayout>
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <label for="thumbnail">New thumbnail:</label>
       <input  style="width: 100%; margin-top: 8px;" type="file" id="thumbnail" accept="image/*" @change="handleFileUpload" :required="!product?.thumbnail_url || product?.thumbnail_url?.length === 0" />
       <br />
  ...
  export default {
    ...
    methods: {
      handleSubmit: async function () {
        ...
        try {
          await api.patch(`/products/${this.$route.params.id}`, data, {
            headers: {
      ...
      getProducts() {
        ...
        .then(response => {
          ...
          this.product.quantity = this.product.quantity.toString();
          this.product.unit_id = this.product.unit_id.toString();
